...
export const Header: FC = memo(() => {
  ...
  return (
    <>
      <Flex as="nav" color="gray.50" align="center"
        justify="space-between" padding={{base: 3, md: 4}} boxShadow='xl' mx={{base: 3, md: 5}}>
        <Flex as="a" role="link" _hover={{ cursor: "pointer" }} onClick={onClickHome} >
          <Heading as="h1" fontSize={{ base: "lg", md: "xl" }} >
            App
          </Heading>
        </Flex>
        <AuthLink loading={loading} isSignedIn={isSignedIn} handleSignout={handleSignout} onClickProfile={onClickProfile}
          onClickLogin={onClickLogin} onClickSignup={onClickSignup} />
      </Flex>
    </>
  );
});
