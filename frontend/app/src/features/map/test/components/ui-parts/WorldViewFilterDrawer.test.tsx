...
import WorldViewFilterDrawer from "features/map/components/ui-parts/WorldViewFilterDrawer";
...
test("FilterDrawerがレンダリングされていること", () => {
  ...
  render(<WorldViewFilterDrawer isOpen onClose={jest.fn()} />);
  expect(screen.getByRole("dialog")).toBeInTheDocument();
});

test("isOpenがfalseの場合、FilterDrawerが非表示であること", () => {
  (mockUseWorldViewListContext as jest.Mock).mockReturnValue(mockContextValue);
  render(<WorldViewFilterDrawer isOpen={false} onClose={jest.fn()} />);
